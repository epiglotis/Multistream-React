      ...
      const res = await axios.get(endpoint).then((response) => {
        ...
        const rename = filterData.map((item) => {
          return {
            ...
            id: item.channelId,
            is_live: item.liveBroadcastContent,
    ...
    try {
      ...
      const res = await axios.get(endpoint).then((response) => {
        if (!Object.keys(response.data.items).length) {
          ...
          const updateData = newRes.map((item) => {
            return {
              user_name: item.channelTitle,
              id: item.channelId,
              icon_url: iconUrl,
              videoId: videoId[0],
              title: item.title,
              thumbnail_url: item.thumbnails.medium.url,
              platform: selectPlatform,
            };
          });
  ...
  return (
    <div className='flex flex-wrap w-[400px]'>
      <div className='w-8/12 h-10'>
        <input
          className='w-full h-10 dark:bg-[#3d3d3f]
      ...
      <div
        className='flex w-10 h-10 justify-center dark:bg-[#2a2a2d]
      ...
      <div className='flex w-2/12 justify-center items-center ml-5'>
        <Select.Root value={selectPlatform} onValueChange={changePlatform}>
          <Select.Trigger
          ...
          <Select.Portal>
            <Select.Content
              className='overflow-hidden bg-white dark:bg-[#2a2a2d]
